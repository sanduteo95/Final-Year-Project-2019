...
const formatResults = testResults => {
    ...
    Object.keys(testResults).forEach(language => {
        const benchmarkResult = testResults[language];
    ...
    return new Promise((resolve, reject) => {
        ...
        return boilerplate.futamura1Boilerplate(code, maxTermCall, false)('pp-' + file)
            .then(futamuraFile0 => {
                ...
                return boilerplate.futamuraBoilerplate(code, maxTermCall, true, 1)(file)
            }).then(futamuraFile1 => {
                ...
                return boilerplate.futamuraBoilerplate(code, maxTermCall, true, 2)('cc-pp-' + file)
            }).then(futamuraFile2 => {
                ...
                return boilerplate.futamuraBoilerplate(code, maxTermCall, true, 3)('pp-cc-' + file)
            }).then(futamuraFile3 => {
                ...
                return boilerplate.futamuraBoilerplate(code, maxTermCall, true, 4)('cc-pp-cc-' + file)
            }).then(futamuraFile4 => {
                ...
                return boilerplate.futamuraBoilerplate(code, maxTermCall, true, 5)('pp-cc-pp-' + file)
            }).then(futamuraFile5 => {
...
const promiseToRunStrategyBenchmark = language => {
    ...
    return files
        .filter(file => file.includes(language === 'EFSD' ? '.efsd' : '.lambda') && !file.includes('read'))
...
const runStrategyBenchmarks = () => {
    return promiseToRunStrategyBenchmark('toyLambda')
        .then(results => {
            ...
            return createStrategyReport({
                'toyLambda': lambdaResults, 
                'EFSD': efsdResults
            });
        });
};
