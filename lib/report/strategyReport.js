...
const promiseToRunAllStrategies = (file, maxTermCall, language) => {
    return new Promise((resolve, reject) => {
        ...
        return boilerplate.futamura1Boilerplate(code, maxTermCall, false)('pp-' + file)
            .then(futamuraFile0 => {
                ...
                return boilerplate.futamura1Boilerplate(code, maxTermCall, true, 1)(file)
            }).then(futamuraFile1 => {
                ...
                return boilerplate.futamura1Boilerplate(code, maxTermCall, true, 2)('cc-pp-' + file)
            }).then(futamuraFile2 => {
                ...
                return boilerplate.futamura1Boilerplate(code, maxTermCall, true, 3)('pp-cc-' + file)
            }).then(futamuraFile3 => {
                ...
                return boilerplate.futamura1Boilerplate(code, maxTermCall, true, 4)('cc-pp-cc-' + file)
            }).then(futamuraFile4 => {
                ...
                return boilerplate.futamura1Boilerplate(code, maxTermCall, true, 5)('pp-cc-pp-' + file)
            }).then(futamuraFile5 => {
...
const promiseToRunStrategyBenchmark = language => {
    ...
    return files
        .filter(file => file.includes(language === 'EFSD' ? '.efsd' : '.lambda') 
                        && !file.includes('read')
                        && file !== 'linear_regression.efsd'
                        && file !== 'random_light_control.efsd'
                        && file !== 'random_planets.efsd'
                        && file !== 'recurrent_neural_network.efsd'
                        && file !== 'tensor_creation.efsd')
...
const runStrategyBenchmarks = () => {
    return promiseToRunStrategyBenchmark('toyLambda')
        .then(results => {
            ...
            return promiseToRunStrategyBenchmark('EFSD')
        }).then(efsdResults => {
            return createStrategyReport([lambdaResults, efsdResults]);
        });
};
