...
const OCaml = path.join(__dirname, '../../../EFSD').replace(/\ /g, '\\ ');
const Agda = path.join(__dirname, '../../../itf-impl/Agda').replace(/\ /g, '\\ ');
...
const getMin = (test, frpResult, jsResult, savedMin) => {
    ...
    if (!savedMin || /* if there is no minimum so far*/
        (frpResult[test] > jsResult.futamura &&
            savedMin[test] > savedMin.futamura) || /* or have found a futamura value that is smaller than the min and comparison */
...
const promiseToRunJS = (file, maxTermCall) => {
    return new Promise((resolve, reject) => {
        ...
        return boilerplate.futamura1Boilerplate('input/' + language + '/' + file, code, maxTermCall, false, undefined, language === 'SSD')
            .then(futamuraFile => {
                futamuraFile = futamuraFile.substring(futamuraFile.indexOf('output'));

                // set up the flag depending on language
                const flag = language === 'SSD' ? '-g' : '';
            ...
            return promiseChain.then(chainResults =>
                currentPromise.then(currentFile => {
                    return new Promise((resolve, reject) => {
                        ...
                        execSync(`sh ${OCaml}/build_example.sh ${fileName} -js > /dev/null`);

                        // TODO: compile Haskell 

                        // compile Agda file into JavaScript
                        execSync(`sh ${Agda}/build_example.sh ${fileName} > /dev/null`);
                        ...
                        suite.add('ocaml', function () {
                            execSync(`${nodeCommand} ${OCaml}/example_build/js/${fileName}.js ${mockInput}`);
                        });
                        ...
                        suite.add('agda', function () {
                            execSync(`${nodeCommand} ${Agda}/Build/${format(fileName)}/jAgda.Examples.${format(fileName)}.js`);
                        });
