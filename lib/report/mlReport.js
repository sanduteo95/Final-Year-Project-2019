...
const promiseToRunJS = (file, maxTermCall) => {
    return new Promise((resolve, reject) => {
        ...
        const code = fs.readFileSync(path.join(__dirname, `../../input/SSD/ML/${file}`), 'utf8');
        return boilerplate.futamura1Boilerplate('input/SSD/ML/pp-' + file, code, maxTermCall, false, undefined, true)
            .then(response => {
                prepackFile = response.substring(response.indexOf('output'));
                return boilerplate.futamura1Boilerplate('input/SSD/ML/cc-' + file, code, maxTermCall, true, 3, true)
            }).then(response => {
                closureFile = response.substring(response.indexOf('output'));
                let suite = new Benchmark.Suite;
                // add tests for both languages
                suite.add('interpreter', function () {
                    execSync(`${nodeCommand} index.js -i input/SSD/ML/${file} -g -s ${maxTermCall}`);
...
const promiseToRunJSBenchmarks = maxTermCall => {
    ...
    return files
        .filter(file => file !== 'neural_network.ssd') // only look at the optimal neural network
