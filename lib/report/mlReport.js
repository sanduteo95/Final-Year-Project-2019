...
const promiseToRunJS = (file, maxTermCall) => {
    return new Promise((resolve, reject) => {
        ...
        return boilerplate.futamura1Boilerplate('input/SSD/ML/' + file, maxTermCall, 0, true)
            .then(response => {
                prepackFile = response.substring(response.indexOf('output'));
                return boilerplate.futamura1Boilerplate('input/SSD/ML/' + file, maxTermCall, 3, true)
            }).then(response => {
