...
const apply1 = (program, callback, file, maxTermCalls, addClosure, closureStrategy, isGoIMachine) => {
    let filePath, fileContents, outputFile;
    if (isGoIMachine) { 
        // set up the input for prepack
        filePath = './SSD/interpreter.js';
        fileContents = fs.readFileSync(path.join(__dirname, filePath), 'utf8');
        fileContents = fileContents.replace('exports["SSD"] =', 'const SSD =');
        fileContents = fileContents + '\nSSD';
        outputFile = '../output/SSD/' + file.substring(file.indexOf('input/SSD/') + 'input/SSD/'.length, file.lastIndexOf('.')) + '.js';
    } else {
        // set up the input for prepack
        filePath = './toyLambda/interpreter.js';
        fileContents = '(' + require(filePath) + ')';
        outputFile = '../output/toyLambda/' + file.substring(file.indexOf('input/toyLambda/') + 'input/toyLambda/'.length, file.lastIndexOf('.')) + '.js';
    }
