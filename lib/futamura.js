...
function apply(program, file, time) { 
    ...
    const {code} = Prepack.prepackSources(
        [{
            ...
            fileContents: interpretFileContents + '(' + JSON.stringify(program) + ', function(err, result) { if (err) { throw err; } else { module.exports = result; }}, ' + time + ');'
        }], prepackOptions);
