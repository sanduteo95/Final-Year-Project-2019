...
const apply1 = (program, callback, file, maxTermCalls, addClosure, closureStrategy) => {
    let filePath, fileContents, outputFile;
    if (file.substring(file.lastIndexOf('.') + 1) === 'efsd') { 
        ...
        fileContents = fileContents + '\nEFSD';
        outputFile = '../output/EFSD/' + file.substring(file.lastIndexOf('/') + 1, file.lastIndexOf('.')) + '.js';
    } else {
        // set up the input for prepack
        filePath = './toyLambda/interpreter.js';
        fileContents = '(' + require(filePath) + ')';
        outputFile = '../output/toyLambda/' + file.substring(file.lastIndexOf('/') + 1, file.lastIndexOf('.')) + '.js';
    }
    fileContents += '(' + JSON.stringify(program) + `,` +  callback.toString() + `, ` + maxTermCalls + `);`;

    if (addClosure) {
        switch (closureStrategy) {
            case 1: // CC
                // just run closure
...
const apply2 = (isGoIMachine, addClosure) => {  
    let prepackFilePath;
    if (isGoIMachine) {
        prepackFilePath = './prepack/EFSD.min.js';
    } else {
        prepackFilePath = './prepack/toyLambda.min.js';
    }
    let prepackFileContents = fs.readFileSync(path.join(__dirname, prepackFilePath), 'utf8');
    const outputFile = isGoIMachine ? './EFSD/compiler.js' : './toyLambda/compiler.js';
    ...
    if (!addClosure) {
        ...
        const {code} = Prepack.prepackSources(
            [{
                filePath: prepackFilePath, 
                fileContents: prepackFileContents
            }], prepackOptions);
        ...
        return new Promise((resolve, reject) => {
            closureCompiler.run([{
                path: prepackFilePath,
                src: prepackFileContents
            }], (exitCode, stdOut, stdErr) => {
                if (exitCode !== 1) {
                    return reject(stdErr);
                }
                fs.writeFileSync(path.join(__dirname, outputFile), stdOut[0].src);
                return resolve(path.join(__dirname, outputFile));    
            }); 
        });
    }
};
