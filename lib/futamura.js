...
function apply(program, callback, file, addTiming, maxTermCalls) { 
    ...
    const {code} = Prepack.prepackSources(
        [{
            ...
            fileContents: interpretFileContents + '(' + JSON.stringify(program) + `,` +  callback(addTiming).toString() + `, ` + addTiming + `, ` + maxTermCalls + `);`}], prepackOptions);
