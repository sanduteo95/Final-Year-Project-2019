...
function futamura1(program, functionApplication, file) { 
    ...
    const {code, map} = Prepack.prepackSources(
        [{
            ...
            fileContents: 'global.result = ' + interpretFileContents + '(' + JSON.stringify(program) + ', ' + JSON.stringify(functionApplication) + ');'
        }], prepackOptions);
