...
function apply(program, file, time, maxTermCalls) { 
    let interpretFilePath = './toyLambda/interpreter.js';
    const interpretFileContents = '(' + require(interpretFilePath) + ')';
 
    const {code} = Prepack.prepackSources(
        [{
            filePath: interpretFilePath, 
            fileContents: interpretFileContents + '(' + JSON.stringify(program) + ', function(err, result) { if (err) { throw err; } else { module.exports = result; }}, ' + time + ', ' + maxTermCalls + ');'
        }], prepackOptions);
