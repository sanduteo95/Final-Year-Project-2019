module.exports = function (program) {
    ...
    function interpret (program) {
        ...
        const address = interpretProgram(program);
    ...
    function interpretVariableDeclaration (variableDeclaration, variableStore, functionStore, isGlobal) {
        ...
        if (!isGlobal) {
            ...
            // point new variable to address
            globalVariableStore[variableDeclaration[1]] = address;
            // if we are passed a function application then we are evaluating a global variable
            return interpretProgram(variableDeclaration[3]);
        }
    }

    function interpretProgram (program) {
        // check first if there are any global variables
        if (program[0] === 'New') {
            return interpretVariableDeclaration(program, [], [], true);
        } else {
            // interpret each function at a time (we will have a list of functions and a function application)
            return interpretFunctions(program[0], program[1], 0, []);
        }
    }

    function interpretFunctions (functionArray, functionApplication, index, functionStore) {
        // the last thing in a list of functions is a function application in a return or not
        if (index === functionArray.length) {
            return interpretStatement(functionApplication, [], functionStore);
        }
        ...
        if (functionDefinition[0] === 'Function') {
            ...
            return interpretFunctions(functionArray, functionApplication, index + 1, functionStore);
        } else {
    ...
    function reorderSequences (mainConstruct, appendedConstruct, restOfBody) {
        ...
        if (mainConstruct[0] !== 'Sequence' && mainConstruct[0] !== 'Let' && mainConstruct[0] !== 'New') {
            if (!restOfBody) {
        ...
        return [mainConstruct[0], mainConstruct[1], reorderSequences(mainConstruct[2], appendedConstruct, restOfBody)];
    }
    ...
    function interpretBody (body, variableStore, functionStore) {
        ...
        switch(body[0]) {
            ...
            case 'New': 
                return interpretVariableDeclaration(body, variableStore, functionStore, false);
            case 'Let':
                return interpretVariableDeclaration(body, variableStore, functionStore, false);
            case 'Loop':
                // check the condition
    ...
    function interpretIdentifier (identifier, variableStore) {
        ...
        const address = variableStore[identifier[1]] || globalVariableStore[identifier[1]];
        if (!address) {
