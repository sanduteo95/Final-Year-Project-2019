    ...
    _createClass(SymBinOp, [{
      ...
      value: function rewrite(token, nextLink) {
        if (token.rewriteFlag == RewriteFlag.F_OP && nextLink.to == this.key) {
          ...
          } else if (this.subType == BinOpType.SymDot) {
            var prevNode = this.graph.findNodeByKey(this.findLinksInto(null)[0].from); // do not replace for now, or we won't be able to update parameters
