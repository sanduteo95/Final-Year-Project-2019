...
const appInterpret = (program, addTiming, maxTermCalls) => {
    EFSD(program, function (result) {
        console.log(result);
        module.exports = result;
    }, addTiming, maxTermCalls);
};

const appFutamura = (program, file, addTiming, maxTermCalls) => {
    ...
    let {code} = Prepack.prepackSources(
        [{
            ...
            fileContents: interpretFileContents + '\nEFSD(' + JSON.stringify(program) + ',function(result){console.log(result);module.exports = result;},' + addTiming + ',' + maxTermCalls + ');'
        }], prepackOptions);
