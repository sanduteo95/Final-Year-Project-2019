module.exports = (program, callback, maxTermCalls) => {        
    ...
    const interpretLambda = (lambda, env, callback) => {
        ...
        for(let i = 0; i < lambda.length - 1; i++) {
            ...
            if (lambda[i].type === 'Def' && lambda[i].value.type !== 'Abstr') {
                ...
                trampoline(interpretTermLazy(lambda[i].value, env, [], (err, address) => { 
                    if (err) {
                        return {
                            fn: callback,
                            args: [err]
    ...
    const interpretTermLazy = (term, env, addressesToBind, callback, isRhsApplication) => {
        ...
        if (global.__residual && termCalls > maxTermCalls) {
            ...
            }, trampoline, interpretTerm, term, env, addressesToBind, callback, isRhsApplication);
        } else {
            return {
                fn: interpretTerm,
                args: [term, env, addressesToBind, callback, isRhsApplication]
                ...
                return {
                    ...
                    args: [null, (...args) => {
                        ...
                        for(let i=0; i<args.length; i++) {
                            argumentAddresses.push(extend(args[i]));
                        }
                        trampoline({
                            fn: interpretAbstraction,
                            args: [result, [], argumentAddresses, lambdaCallback, true]
                        });
                    }]
                };
            } else {
