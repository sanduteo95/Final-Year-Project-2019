module.exports = function (program, callback, addTiming, maxTermCalls) {
    ...
    const interpretLambda = function (lambda, env, callback) {
        if (addTiming) {
            if (global.__residual) {
        ...
        for(let i = 0; i < lambda.length - 1; i++) {
            ...
            if (lambda[i].type === 'Def' && lambda[i].value.type !== 'Abstr') {
                ...
                interpretTerm(lambda[i].value, env, [], function (err, address) {
                    if (err) {
                        callback(err);
                    } else {
        ...
        interpretTerm(lambda[lambda.length - 1], env, [], callback, false);
    }

    const interpretTerm = function (term, env, addressesToBind, callback, isRhsApplication) {
        ...
        if (global.__residual) {
            ...
            if (termCalls > maxTermCalls/9) {
                ...
                if (termCalls > maxTermCalls) {
                    ...
                    }, global.setTimeout, interpretTermLazy, term, env, addressesToBind, callback, isRhsApplication);
                } else {
    ...
    const interpretTermLazy = function (term, env, addressesToBind, callback, isRhsApplication) {
        ...
        switch(term.type) {
            case 'Deref':
                interpretDereference(term.value, env, addressesToBind, termCallback, isRhsApplication);
                break;
            case 'Const':
                interpretConstant(term.value, callback);  
                break;
            case 'Op':
                interpretOperator(term, env, addressesToBind, termCallback, isRhsApplication);
                break;
            case 'Abstr':
                interpretAbstraction(term, env, addressesToBind, termCallback, isRhsApplication);
                break;
            case 'Apply':
                interpretApplication(term.value, env, addressesToBind, termCallback, isRhsApplication);
                break;
            default:
    ...
    const interpretAbstraction = function (abstraction, env, addressesToBind, callback, isRhsApplication) {
        ...
        interpretTerm(abstraction.value, newEnv, addressesToBind, abstrCallback, true);
    }

    const interpretApplication = function (application, env, addressesToBind, callback, isRhsApplication) {
    ...
    const interpretIdentifier = function (identifier, env, callback) {
    ...
    const interpretDereference = function (dereference, env, addressesToBind, callback, isRhsApplication) {
                ...
                if (address === printAddress) {
                    ...
                    if (global.__residual && global.__isAbstract(lookup(address))) {
                        ...
                        if (newTerm && (newTerm.type === 'Abstr' ||  newTerm.type === 'Apply') && (!isRhsApplication || !memoized[dereference.value])) {
                            ...
                            interpretTerm(newTerm, env, addressesToBind, callback, true);
                        } else {
        ...
        interpretIdentifier(dereference.value, env, derefCallback);
    }
    ...
    const interpretOperator = function (operator, env, addressesToBind, callback, isRhsApplication) {
        ...
        const lhsOpCallback = function (err, lhsAddress) {
            ...
            if (operator.op !== 'negate' && operator.op !== 'negative') {
                ...
                if ((operator.op === 'or' && lookup(lhsAddress)) || 
                    ...
                    interpretTerm(operator.rhs, env, addressesToBind, callback, isRhsApplication);
                } else {
                    interpretTerm(operator.rhs, env, addressesToBind, rhsOpCallback, isRhsApplication);
                }
            } else {
                rhsOpCallback(null, emptyAddress);
            }
        };
        interpretTerm(operator.lhs, env, addressesToBind, lhsOpCallback, isRhsApplication);
    }
