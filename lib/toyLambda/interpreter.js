module.exports = function (program, callback, addTiming, maxTermCalls) {
    ...
    if (global.__residual) {
        ...
        abstractedDate = __abstract({}, 'new Date()');
    }
    ...
    const interpretLambda = function (lambda, boundVariables, callback) {
        if (addTiming) {
            if (global.__residual) {
                startTime = global.__residual('number', function (date) {
                    return date;
                }, abstractedDate);
            } else {
                startTime = new Date();
    ...
    const lambdaCallback = function (err, address) {
        ...
        if (addTiming) {
            if (global.__residual) {
                timing = global.__residual('number', function (date, startTime) {
                    return date - startTime;
                }, abstractedDate, startTime);
            } else {
        ...
        if (global.__isAbstract && global.__isAbstract(result)) {
            ...
            }, abstractedConsole, waitForInput, address === printAddress, lookup, callback, timing);
        } else {
            if (result !== undefined && result.type === 'Abstr') {
                ...
                callback(err, address > printAddress && lookup(address), timing);
            }
        }
    }
