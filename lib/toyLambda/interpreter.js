module.exports = function (program, callback) {
    ...
    function interpretTermLazy (term, boundVariables, addressesToBind, callback, isAbstraction) {
        ...
        switch(term[0]) {
            case 'Deref':
                interpretDereference(term[1], boundVariables, addressesToBind, callback, isAbstraction)
                break;
            case 'Const':
            ...
            if (err) {
                ...
                const lhsCallback = function (err, newAddress) {
                    if (err) {
                        ...
                        if ((!global.__isAbstract || !global.__isAbstract(stack[newAddress])) && stack[newAddress] && stack[newAddress][0] === 'Deref') {
                            logDebug && console.log('$CHECK: The new LHS is a dereference, so check if it was replaced by something else')
                            if (application[0][0] === 'Deref' && application[0][1][1] === stack[newAddress][1][1]) {
    ...
    function interpretDereference (dereference, boundVariables, addressesToBind, callback, isAbstraction) {
        ...
        const derefCallback = function (err, address) {
            if (err) {
                ...
                if(!isAbstraction) {
                    ...
                    stack[++stackAddress] = ['Deref', dereference];
                    callback(null, stackAddress);
                }
            } else {
                ...
                if (address === printAddress) {
                    ...
                    if (global.__residual && global.__isAbstract(stack[address])) {
                        ...
                        if (newTerm && (newTerm[0] === 'Abstr' ||  newTerm[0] === 'Apply')) {
                            logDebug && console.log('$ATTENTION: Variable ' + term[1][1] + ' holds an application')
                            ...
                            interpretTerm(newTerm, scopedBoundVariables, addressesToBind, callback, newTerm[0] === 'Abstr');
                        } else {
        ...
        interpretIdentifier(dereference[1], boundVariables, derefCallback);
    }
    ...
    function applyOperator (operator, lhsAddress, rhsAddress, callback) {
        let err = null;
        // check if both the LHS and RHS are constants
