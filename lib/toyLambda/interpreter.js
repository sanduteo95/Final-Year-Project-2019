module.exports = function (program, callback, addTiming, maxTermCalls) {    
    ...
    const interpretLambda = function (lambda, env, callback) {
        ...
        for(let i = 0; i < lambda.length - 1; i++) {
            ...
            if (lambda[i].type === 'Def' && lambda[i].value.type !== 'Abstr') {
                ...
                trampoline(interpretTerm(lambda[i].value, env, [], function (err, address) {
                    if (err) {
                        return {
                            func: callback,
                            args: [err]
    ...
    const interpretTerm = function (term, env, addressesToBind, callback, isRhsApplication) {
        ...
        if (global.__residual && termCalls > maxTermCalls) {
            global.__residual("void", function (trampoline, interpretTermLazy, term, env, addressesToBind, callback, isRhsApplication) {
                return trampoline({
    ...
    const interpretApplication = function (application, env, addressesToBind, callback) {
