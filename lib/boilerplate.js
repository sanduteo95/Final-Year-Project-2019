...
module.exports = {
    ...
    futamuraBoilerplate: (code, maxTermCalls) => {
        return fileName => {
            if (fileName.substring(fileName.lastIndexOf('.') + 1) === 'efsd') {
                return futamura.apply(code, callback, fileName, maxTermCalls);
            } else {
                return futamura.apply(parser.parse(code), callback, fileName, maxTermCalls);
            }        
        };
    },
