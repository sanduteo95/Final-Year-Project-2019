(function () {
  var __get_scope_binding_0 = function (__selector) {
    var __captured;

    switch (__selector) {
      case 0:
        __captured = [10];
        break;
    }

    __scope_0[__selector] = __captured;
    return __captured;
  };

  var __scope_0 = new Array(1);

  var _24 = function (err, result) {
    if (err) {
      throw err;
    } else {
      module.exports = result;
    }
  };

  var _1k = function (term, boundVariables, variablesToBound, callback, isApplication) {
    switch (term[0]) {
      case 'Deref':
        _1m(term[1], boundVariables, callback);

        break;

      case 'Const':
        _1o(term[1], callback);

        break;

      case 'Op':
        _1p(term[1], boundVariables, variablesToBound, callback, isApplication);

        break;

      case 'Abstr':
        _1q(term[1], boundVariables, variablesToBound, callback, isApplication);

        break;

      case 'Apply':
        _1r(term[1], boundVariables, variablesToBound, callback, isApplication);

        break;

      default:
        callback(new Error('Failed to interpret term for ' + term[0]), null);
        return;
    }
  };

  var _1q = function (abstraction, boundVariables, variablesToBound, callback, isApplication) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);

    // bind variable if there is anything to bind
    if (variablesToBound.length > 0) {
      const numCalls = !boundVariables[abstraction[0]] ? 0 : boundVariables[abstraction[0]].numCalls;
      boundVariables[abstraction[0]] = {
        address: variablesToBound.pop(),
        numCalls: numCalls + 1
      };
    }

    if (global.__residual && boundVariables[abstraction[0]] && boundVariables[abstraction[0]].numCalls > 2) {
      __residual("void", _1j.bind(null), _2, __captured__scope_1[0], _1k, abstraction, boundVariables, variablesToBound, callback, isApplication);
    } else {
      _1k(abstraction[1], boundVariables, variablesToBound, function (err, address) {
        // if there is an error
        if (err) {
          // check if the error is because abstract variable is not defined
          if (isApplication && err.message === 'Variable or named lambda ' + abstraction[0] + ' has not been defined') {
            __captured__scope_1[0] = __captured__scope_1[0] + 1;
            _2[__captured__scope_1[0]] = ['Abstr', abstraction];
            address = __captured__scope_1[0];
          } else {
            // otherwise callback the error
            callback(err, null);
            return;
          }
        } // pass the address to the callback


        callback(null, address);
      }, isApplication);
    }
  };

  var _1r = function (application, boundVariables, variablesToBound, callback, isApplication) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);

    const rhsCallback = function (err, address) {
      if (err) {
        callback(err, null);
        return;
      }

      variablesToBound.push(address);

      const lhsCallback = function (err, newAddress) {
        if (err) {
          callback(err, null);
          return;
        } // clean up stack


        if (address !== newAddress) {
          _2[address] = _2[newAddress];
        }

        __captured__scope_1[0] = address;
        callback(null, address);
      }; // check if the LHS is a normal abstraction or a variable


      switch (application[0][0]) {
        case 'Abstr':
        case 'Apply':
          _1k(application[0], boundVariables, variablesToBound, lhsCallback, isApplication);

          break;

        case 'Deref':
          _1k(application[0], boundVariables, variablesToBound, function (err, termAddress) {
            if (err) {
              callback(err, null);
              return;
            }

            const term = _2[termAddress];

            if (term[0] !== 'Abstr') {
              callback(new Error('Failed to interpret application for named lambda ' + application[0][1][1] + ' which stores a ' + term[0]), null);
              return;
            }

            _1k(term, boundVariables, variablesToBound, lhsCallback, isApplication);
          }, isApplication);

          break;

        default:
          callback(new Error('Failed to interpret application for ' + application[0][0]), null);
          return;
      }
    }; // interpret the RHS termand add it to the variablesToBound array 
    // send an empty array of variables to bind because it's a different scope


    _1k(application[1], boundVariables, [], rhsCallback, true);
  };

  var _1w = function (callback) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);

    const rl = _1x.createInterface({
      input: process.stdin,
      output: process.stdout
    });

    rl.question('User input:\n', function (input) {
      rl.close();
      __captured__scope_1[0] = __captured__scope_1[0] + 1;
      _2[__captured__scope_1[0]] = input;
      callback(null, __captured__scope_1[0]);
    });
  };

  var _1t = function (identifier, boundVariables, callback) {
    // just get the address on the stack pointed at by the identifier
    const address = !boundVariables[identifier] ? undefined : boundVariables[identifier].address;

    if (!address) {
      callback(new Error('Variable or named lambda ' + identifier + ' has not been defined'), null);
      return;
    }

    if (identifier === '_print') {
      _2[1] = _2[address];
      callback(null, 1);
    } else if (identifier === '_read') {
      // for prepack
      if (global.__residual) {
        __residual("void", function (waitForInput, callback) {
          waitForInput(callback);
        }, _1w, callback);
      } else {
        _1w(callback);
      }
    } else {
      callback(null, address);
    }
  };

  var _1m = function (dereference, boundVariables, callback) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);

    if (dereference[0] !== 'Identifier') {
      callback(new Error('Failed to interpreter dereference for ' + dereference[0]), null);
      return;
    } // get address of the identifier on the stack


    _1t(dereference[1], boundVariables, function (err, address) {
      if (err) {
        callback(err, null);
        return;
      }

      if (address === 1) {
        console.log(_2[1]);
        callback(null, 1);
      } else {
        // increase the stack address and store the value of the identifier there
        __captured__scope_1[0] = __captured__scope_1[0] + 1;
        _2[__captured__scope_1[0]] = _2[address];
        callback(null, __captured__scope_1[0]);
      }
    });
  };

  var _1o = function (constant, callback) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);

    // just increment the address on the stack and assign it the constant value
    __captured__scope_1[0] = __captured__scope_1[0] + 1;
    _2[__captured__scope_1[0]] = constant;
    callback(null, __captured__scope_1[0]);
  };

  var _1v = function (operator, lhsAddress, rhsAddress, callback) {
    switch (operator) {
      case 'Plus':
        _2[lhsAddress] = _2[lhsAddress] + _2[rhsAddress];
        break;

      case 'Minus':
        _2[lhsAddress] = _2[lhsAddress] - _2[rhsAddress];
        break;

      case 'Times':
        _2[lhsAddress] = _2[lhsAddress] * _2[rhsAddress];
        break;

      case 'Divide':
        _2[lhsAddress] = _2[lhsAddress] / _2[rhsAddress];
        break;

      case 'Modulus':
        _2[lhsAddress] = _2[lhsAddress] % _2[rhsAddress];
        break;

      case 'Eq':
        _2[lhsAddress] = _2[lhsAddress] == _2[rhsAddress];
        break;

      case 'Noteq':
        _2[lhsAddress] = _2[lhsAddress] != _2[rhsAddress];
        break;

      case 'Leq':
        _2[lhsAddress] = _2[lhsAddress] <= _2[rhsAddress];
        break;

      case 'Less':
        _2[lhsAddress] = _2[lhsAddress] < _2[rhsAddress];
        break;

      case 'Geq':
        _2[lhsAddress] = _2[lhsAddress] >= _2[rhsAddress];
        break;

      case 'Greater':
        _2[lhsAddress] = _2[lhsAddress] > _2[rhsAddress];
        break;

      case 'And':
        _2[lhsAddress] = _2[lhsAddress] && _2[rhsAddress];
        break;

      case 'Or':
        _2[lhsAddress] = _2[lhsAddress] || _2[rhsAddress];
        break;

      case 'Negate':
        _2[lhsAddress] = !_2[lhsAddress];
        break;

      case 'Negative':
        _2[lhsAddress] = -_2[lhsAddress];
        break;

      default:
        callback(new Error('Failed to interpret operator for ' + operator[0]), null);
        return;
    }

    callback(null, lhsAddress);
  };

  var _1p = function (operator, boundVariables, variablesToBound, callback, isApplication) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);

    const lhsOpCallback = function (err, lhsAddress) {
      if (err) {
        callback(err, null);
        return;
      } // clean up stack


      __captured__scope_1[0] = lhsAddress;

      const rhsOpCallback = function (err, rhsAddress) {
        if (err) {
          callback(err, null);
          return;
        }

        _1v(operator[0], lhsAddress, rhsAddress, callback);
      };

      if (operator[0] !== 'Negate' && operator[0] !== 'Negative') {
        // don't interpret stuff if not needed to
        if (operator[0] === 'Or' && _2[lhsAddress] || operator[0] === 'And' && !_2[lhsAddress]) {
          callback(null, lhsAddress);
        } else {
          _1k(operator[2], boundVariables, variablesToBound, rhsOpCallback, isApplication);
        }
      } else {
        rhsOpCallback(null, undefined);
      }
    };

    _1k(operator[1], boundVariables, variablesToBound, lhsOpCallback, isApplication);
  };

  var _23 = function (err, address) {
    // if there is an error, throw it
    if (err) {
      _24(err, null);

      return;
    } // otherwise, return the result
    // if the address is empty then it returns undefined


    if (address != 1) {
      _24(null, _2[address]);
    } else {
      _24(null, _2[0]);
    }
  };

  var _22 = function (err, address) {
    // if there is an error run callback with it
    if (err) {
      _23(err, null);

      return;
    } // otherwise run callback with the address


    _23(null, address);
  };

  var $_0 = function (callback, address, __scope_1, err, newAddress) {
    var __captured__scope_1 = __scope_0[__scope_1] || __get_scope_binding_0(__scope_1);

    if (err) {
      callback(err, null);
      return;
    }

    if (address !== newAddress) {
      _2[address] = _2[newAddress];
    }

    __captured__scope_1[0] = address;
    callback(null, address);
  };

  var _1l = function (err, newAddress) {
    return $_0.call(this, _1n, 9, 0, err, newAddress);
  };

  var _1z = function (err, newAddress) {
    return $_0.call(this, _21, 6, 0, err, newAddress);
  };

  var _21 = function (err, newAddress) {
    return $_0.call(this, _22, 5, 0, err, newAddress);
  };

  var $_1 = function (abstraction, callback, __scope_1, err, address) {
    var __captured__scope_1 = __scope_0[__scope_1] || __get_scope_binding_0(__scope_1);

    if (err) {
      if (void 0) {
        __captured__scope_1[0] = __captured__scope_1[0] + 1;
        _2[__captured__scope_1[0]] = ['Abstr', abstraction];
        address = __captured__scope_1[0];
      } else {
        callback(err, null);
        return;
      }
    }

    callback(null, address);
  };

  var _1u = function (err, address) {
    return $_1.call(this, _P, _1y, 0, err, address);
  };

  var _1y = function (err, address) {
    return $_1.call(this, _L, _1z, 0, err, address);
  };

  var _1n = function (err, rhsAddress) {
    if (err) {
      _1u(err, null);

      return;
    }

    _1v(_T[0], 8, rhsAddress, _1u);
  };

  var _1j = function (stack, stackAddress, interpretTerm, abstraction, boundVariables, variablesToBound, callback, isApplication) {
    interpretTerm(abstraction[1], boundVariables, variablesToBound, function (err, address) {
      // if there is an error
      if (err) {
        // check if the error is because abstract variable is not defined
        if (isApplication && err.message === 'Variable or named lambda ' + abstraction[0] + ' has not been defined') {
          stackAddress = stackAddress + 1;
          stack[stackAddress] = ['Abstr', abstraction];
          address = stackAddress;
        } else {
          // otherwise callback the error
          callback(err, null);
          return;
        }
      } // pass the address to the callback


      callback(null, address);
    }, isApplication);
  };

  console.log(1);
  var _D = ["Identifier", "x"];
  var _B = ["Deref", _D];
  var _G = ["Const", 1];
  var _9 = ["Plus", _B, _G];
  var _7 = ["Op", _9];
  var _5 = ["x", _7];
  var _3 = ["Abstr", _5];
  var _i = ["Identifier", "_print"];
  var _g = ["Deref", _i];
  var _e = ["_print", _g];
  var _c = ["Abstr", _e];
  var _n = ["Identifier", "x"];
  var _l = ["Deref", _n];
  var _b = [_c, _l];
  var _Z = ["Apply", _b];
  var _w = ["Identifier", "x"];
  var _u = ["Deref", _w];
  var _z = ["Const", 10];
  var _s = ["Less", _u, _z];
  var _q = ["Op", _s];
  var _X = ["And", _Z, _q];
  var _V = ["Op", _X];
  var _17 = ["Identifier", "f"];
  var _15 = ["Deref", _17];
  var _1F = ["Identifier", "incr"];
  var _1D = ["Deref", _1F];
  var _1K = ["Identifier", "x"];
  var _1I = ["Deref", _1K];
  var _1C = [_1D, _1I];
  var _1A = ["Apply", _1C];
  var _14 = [_15, _1A];
  var _12 = ["Apply", _14];
  var _T = ["And", _V, _12];
  var _R = ["Op", _T];
  var _P = ["f", _R];
  var _N = ["Abstr", _P];
  var _L = ["x", _N];
  var _J = ["Abstr", _L];
  var _2 = [, 1,, _3, _J, _J, 1, _J, true, 2, _J, 2, 1];

  var _1x = require("readline");

  var _1g = [,,];
  _1g.length = 0;

  _1j(_2, 10, _1k, _L, {
    _print: {
      address: 8,
      numCalls: 0 / 0
    },
    _read: {
      address: 2
    },
    incr: {
      address: 3
    },
    rec: {
      address: 4
    },
    x: {
      address: 9,
      numCalls: 3
    },
    f: {
      address: 5,
      numCalls: 1
    }
  }, _1g, _1l, void 0);
})();