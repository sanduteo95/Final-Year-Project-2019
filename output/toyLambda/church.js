(function () {
  ...
  var _2u_ = function (term, boundVariables, addressesToBind, callback, isRhsApplication) {
    var __captured__scope_1_ = __scope_0_main[0] || __get_scope_binding_0_get_95scope_95binding(0);
    ...
    if (global.__residual) {
      ...
      if (__captured__scope_1_[0] > 125 / 9) {
        ...
        if (__captured__scope_1_[0] > 125) {
          ...
          global.__residual("void", _V_.bind(null), _W_, term, boundVariables, addressesToBind, callback, isRhsApplication);
        }
      } else {
  ...
  var _W_ = function (term, boundVariables, addressesToBind, callback, isRhsApplication) {
    ...
    switch (term.type) {
      ...
      case 'Abstr':
        _e_(term, boundVariables, addressesToBind, termCallback, isRhsApplication);

        break;

      case 'Apply':
        _f_(term.value, boundVariables, addressesToBind, termCallback, isRhsApplication);
  ...
  var _e_ = function (abstraction, boundVariables, addressesToBind, callback, isRhsApplication) {
    false;
    ...
    if (addressesToBind.length > 0) {
      ...
      newBoundVariables[abstraction.binding] = _2t_({
        type: 'Deref',
        value: {
          type: 'Identifier',
    ...
    _2u_(abstraction.value, newBoundVariables, addressesToBind, abstrCallback, true);
  };

  var _f_ = function (application, boundVariables, addressesToBind, callback, isRhsApplication) {
    var __captured__scope_1_ = __scope_0_main[0] || __get_scope_binding_0_get_95scope_95binding(0);
    ...
    const rhsCallback = function (err, rhsAddress) {
      if (err) {
        ...
        _2u_(application.lhs, boundVariables, addressesToBind, lhsCallback, false);
      }
    };

    false; // interpret the RHS term with an empty array of addresses to bind because it's a different scope

    _2u_(application.rhs, boundVariables, [], rhsCallback, true);
  };
  ...
  var _2w_ = function (identifier, boundVariables, callback) {
  ...
  var _b_ = function (dereference, boundVariables, addressesToBind, callback, isRhsApplication) {
        ...
        if (address === 1) {
          ...
          if (global.__residual && global.__isAbstract(_a_(address))) {
            ...
            if (newTerm && (newTerm.type === 'Abstr' || newTerm.type === 'Apply') && (!isRhsApplication || !_2q_[dereference.value])) {
              ...
              _2u_(newTerm, boundVariables, addressesToBind, callback, true);
            } else {
              false;
    ...
    _2w_(dereference.value, boundVariables, derefCallback);
  };
  ...
  var _d_ = function (operator, boundVariables, addressesToBind, callback, isRhsApplication) {
    ...
    const lhsOpCallback = function (err, lhsAddress) {
      ...
      if (operator.op !== 'negate' && operator.op !== 'negative') {
        ...
        if (operator.op === 'or' && _a_(lhsAddress) || operator.op === 'and' && !_a_(lhsAddress)) {
          ...
          _2u_(operator.rhs, boundVariables, addressesToBind, callback, isRhsApplication);
        } else {
          _2u_(operator.rhs, boundVariables, addressesToBind, rhsOpCallback, isRhsApplication);
        }
      } else {
    ...
    _2u_(operator.lhs, boundVariables, addressesToBind, lhsOpCallback, isRhsApplication);
  };
  ...
  var _3u_ = function (err, address) {
    ...
    if (global.__isAbstract && global.__isAbstract(result)) {
      ...
      }, _3x_, _34_, address === 1, _a_, _3y_, timing);
    } else {
      if (result !== undefined && result.type === 'Abstr') {
        ...
        _3y_(err, address > 1 && _a_(address), timing);
      }
    }
  };
  ...
  var _V_ = function (interpretTermLazy, term, boundVariables, addressesToBind, callback, isRhsApplication) {
    interpretTermLazy(term, boundVariables, addressesToBind, callback, isRhsApplication);
  };
