(function () {
  ...
  var _T = function (term, boundVariables, variablesToBound, callback, isApplication) {
    switch (term[0]) {
      case 'Deref':
        _V(term[1], boundVariables, callback);
      ...
      case 'Op':
        _Y(term[1], boundVariables, variablesToBound, callback, isApplication);

        break;

      case 'Abstr':
        _Z(term[1], boundVariables, variablesToBound, callback, isApplication);

        break;

      case 'Apply':
        _a(term[1], boundVariables, variablesToBound, callback, isApplication);
  ...
  var _Z = function (abstraction, boundVariables, variablesToBound, callback, isApplication) {
    ...
    if (global.__residual && boundVariables[abstraction[0]] && boundVariables[abstraction[0]].numCalls > 2) {
      ...
      }, _5, __captured__scope_1[0], _T, abstraction, boundVariables, variablesToBound, callback, isApplication);
    } else {
      _T(abstraction[1], boundVariables, variablesToBound, function (err, address) {
        // if there is an error
        if (err) {
          // check if the error is because abstract variable is not defined
          if (isApplication && err.message === 'Variable or named lambda ' + abstraction[0] + ' has not been defined') {
  ...
  var _a = function (application, boundVariables, variablesToBound, callback, isApplication) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);
    ...
    const rhsCallback = function (err, address) {
      ...
      switch (application[0][0]) {
        case 'Abstr':
        case 'Apply':
          _T(application[0], boundVariables, variablesToBound, lhsCallback, isApplication);

          break;

        case 'Deref':
          _T(application[0], boundVariables, variablesToBound, function (err, termAddress) {
            if (err) {
              callback(err, null);
              return;
            }
            ...
            _T(term, boundVariables, variablesToBound, lhsCallback, isApplication);
          }, isApplication);
    ...
    _T(application[1], boundVariables, [], rhsCallback, true);
  };
  ...
  var _c = function (identifier, boundVariables, callback) {
  ...
  var _V = function (dereference, boundVariables, callback) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);
    ...
    _c(dereference[1], boundVariables, function (err, address) {
      if (err) {
        callback(err, null);
        return;
      }
  ...
  var _d = function (operator, lhsAddress, rhsAddress, callback) {
    switch (operator) {
      case 'Plus':
  ...
  var _Y = function (operator, boundVariables, variablesToBound, callback, isApplication) {
    var __captured__scope_1 = __scope_0[0] || __get_scope_binding_0(0);
    ...
    const lhsOpCallback = function (err, lhsAddress) {
      ...
      const rhsOpCallback = function (err, rhsAddress) {
        ...
        _d(operator[0], lhsAddress, rhsAddress, callback);
      };
      ...
      if (operator[0] !== 'Negate' && operator[0] !== 'Negative') {
        ...
        if (operator[0] === 'Or' && _5[lhsAddress] || operator[0] === 'And' && !_5[lhsAddress]) {
          ...
          _T(operator[2], boundVariables, variablesToBound, rhsOpCallback, isApplication);
        }
      } else {
    ...
    _T(operator[1], boundVariables, variablesToBound, lhsOpCallback, isApplication);
  };

  var _U = function (err, address) {
  ...
  var _7 = function (err, address) {
    ...
    switch (_A[0][0]) {
      case 'Abstr':
      case 'Apply':
        _T(_A[0], _P, _9, lhsCallback, void 0);

        break;

      case 'Deref':
        _T(_A[0], _P, _9, function (err, termAddress) {
          if (err) {
            _8(err, null);
          ...
          _T(term, _P, _9, lhsCallback, void 0);
        }, void 0);
  ...
  var _5 = [];
  var _9 = [];
  var _A = [["Abstr", ["x", ["Deref", ["Identifier", "x"]]]], ["Deref", ["Identifier", "_read"]]];
  var _P = {
    _print: {
      address: 1
    },
